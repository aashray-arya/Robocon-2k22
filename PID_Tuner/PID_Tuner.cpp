...
bool PIDTuner::readBytes(byte data[], byte toRead, char toSend)
{
    ...
    {
        byte x = port.read();
        // debug_msg("byte Received");
        lastRead = millis();
        if (x == startMarker)
        {
            // debug_msg("start byte Received");
            inProgress = true;
            bytesRecvd = 0;
        }
        if (inProgress)
        ...
        if (x == endMarker)
        {
            ...
            if (decodedByteCount == toRead)
                ...
                debug_msg("Received Incorrect Bytes in :: " + String(int(toSend)));
        }
    }
    if ((millis() - lastRead) > 50)
    {
        debug_msg("SERIAL TIMEOUT");
        firstCall = true;
    }
    return false;
}

void PIDTuner::debug_msg(String errorMessage)
{
